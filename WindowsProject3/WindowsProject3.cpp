            ...
            LPARAM lParam)
{
    ...
    static HWND hDatePicker1;
    static HWND hLabelDate1;
    ...
    case WM_INITDIALOG:
        ...
        HWmascota = GetDlgItem(hDlg, IDC_EDIT3);
        HWconsulta = GetDlgItem(hDlg, IDC_EDIT4);
        HWcosto = GetDlgItem(hDlg, IDC_EDIT5);
        HWestatus = GetDlgItem(hDlg, IDC_EDIT6);
        hDatePicker1 = GetDlgItem(hDlg, IDC_DATETIMEPICKER1);
        hLabelDate1 = GetDlgItem(hDlg, IDC_FECHA1);
        SendMessage(HWtel, WM_SETTEXT, 0, (LPARAM)"captura telefono");
        SendMessage(HWnombre, WM_SETTEXT, 0, (LPARAM)"captura nombre");
        SendMessage(HWmascota, WM_SETTEXT, 0, (LPARAM)"captura nombre");
        SendMessage(HWconsulta, WM_SETTEXT, 0, (LPARAM)"captura motivo");
        SendMessage(HWcosto, WM_SETTEXT, 0, (LPARAM)"captura costo");
        SendMessage(HWestatus, WM_SETTEXT, 0, (LPARAM)"captura estatus");
    ...
    case WM_COMMAND:
        ...
        if (LOWORD(wParam) == IDC_LEERFECHA && HIWORD(wParam) == BN_CLICKED)
        {
            char buffer[80];
            GetWindowText(hDatePicker1, (LPTSTR)buffer, 80);
            SetWindowText(hLabelDate1, buffer);
        ...
        {            
            ...
            SendMessage(HWmascota, WM_GETTEXT, 20, (LPARAM)alumno_temp.mascota);
            SendMessage(HWconsulta, WM_GETTEXT, 30, (LPARAM)alumno_temp.motivo);
            SendMessage(HWestatus, WM_GETTEXT, 20, (LPARAM)alumno_temp.estatus);
            
            if (strlen(matricula) == 0 || strlen(costo) == 0 || strlen(alumno_temp.nombre) == 0 ||
                strlen(alumno_temp.mascota) == 0 || strlen(alumno_temp.motivo) == 0 || strlen(alumno_temp.estatus) == 0)
            {
            ...
            strcpy(nuevo->mascota, alumno_temp.mascota);
            strcpy(nuevo->motivo, alumno_temp.motivo);
            strcpy(nuevo->estatus, alumno_temp.estatus);
            ...
            // prepara el mensaje
            strcpy(mensaje, "Cita de: ");
            strcat(mensaje, alumno_temp.nombre);
            strcat(mensaje, ", ha sido agregada");

            MessageBox(0, mensaje, "AGREGAR CITA",
            ...
            SendMessage(HWmascota, WM_SETTEXT, 0, (LPARAM)"");
            SendMessage(HWconsulta, WM_SETTEXT, 0, (LPARAM)"");
            SendMessage(HWestatus, WM_SETTEXT, 0, (LPARAM)"");
...
INT_PTR CALLBACK ListaAlumnos(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    static HWND hDatePicker;
    static HWND hLabelDate;
    static HWND hEditMatricula;
    static HWND hEditMascota;
    static HWND hEditMotivo;
    static HWND hEditCosto;
    static HWND hEditEstatus;

    switch (message)
    {
    case WM_INITDIALOG:
        hDatePicker = GetDlgItem(hDlg, IDC_DATETIMEPICKER1);
        hLabelDate = GetDlgItem(hDlg, IDC_FECHA);
        hEditMatricula = GetDlgItem(hDlg, IDC_EDITMATR);
        hEditMascota = GetDlgItem(hDlg, IDC_EDITMASC);
        hEditMotivo = GetDlgItem(hDlg, IDC_EDITMOTIVO);
        hEditCosto = GetDlgItem(hDlg, IDC_EDITCOSTO);
        hEditEstatus = GetDlgItem(hDlg, IDC_EDITESTATUS);
    ...
    case WM_COMMAND:
        ...
        {
            SendMessage(hEditMatricula, WM_GETTEXT, 20, (LPARAM)s_matr);
            SendMessage(hEditCosto, WM_GETTEXT, 20, (LPARAM)Costo);
            SendMessage(hEditMascota, WM_GETTEXT, 20, (LPARAM)alumno_temp.mascota);
            SendMessage(hEditMotivo, WM_GETTEXT, 20, (LPARAM)alumno_temp.motivo);
            SendMessage(hEditEstatus, WM_GETTEXT, 20, (LPARAM)alumno_temp.estatus);
            
            alumno_temp.matricula = atoi(s_matr);
            alumno_temp.costo = atoi(Costo);

            nodo_alumnos* actual = lista_alumnos;
            while (actual != nullptr && actual->matricula != alumno_temp.matricula)
            {
                actual = actual->sig;
            }
            ...
            if (actual != nullptr)
            {
                ...
                actual->costo = alumno_temp.costo;
                strcpy(actual->mascota, alumno_temp.mascota);
                strcpy(actual->motivo, alumno_temp.motivo);
                strcpy(actual->estatus, alumno_temp.estatus);

                // Asignar género según los botones de radio
                if (IsDlgButtonChecked(hDlg, IDC_RADIO1) == BST_CHECKED)
                    strcpy(actual->genero, "H");
                else if (IsDlgButtonChecked(hDlg, IDC_RADIO2) == BST_CHECKED)
                    strcpy(actual->genero, "M");
                else if (IsDlgButtonChecked(hDlg, IDC_RADIO3) == BST_CHECKED)
                    strcpy(actual->genero, "C");
            }
            
            //limpiar las cajitas de texto
            
            
            SendMessage(hEditCosto, WM_SETTEXT, 0, (LPARAM)"");
            SendMessage(hEditMatricula, WM_SETTEXT, 0, (LPARAM)"");
            SendMessage(hEditMascota, WM_SETTEXT, 0, (LPARAM)"");
            SendMessage(hEditMotivo, WM_SETTEXT, 0, (LPARAM)"");
            SendMessage(hEditEstatus, WM_SETTEXT, 0, (LPARAM)"");
        ...
        if (LOWORD(wParam) == IDC_OK)
        {
            ...
            MessageBox(0, nombre, "LISTA CITAS", MB_ICONINFORMATION + MB_OK);
            ...
            if (temp != 0)
            {
                ...
                strcpy(mascota, temp->mascota);
                strcpy(Motivo, temp->motivo);
                strcpy(Estatus, temp->estatus);
                _itoa(temp->costo, Costo, 10);

                // Asigna la matrícula y la mascota a los controles de edición
                SetWindowText(hEditMatricula, s_matr);
                SetWindowText(hEditMascota, mascota);
                SetWindowText(hEditMotivo, Motivo);
                SetWindowText(hEditCosto, Costo);
                SetWindowText(hEditEstatus, Estatus);
        ...
        if (LOWORD(wParam) == IDC_GETDATE && HIWORD(wParam) == BN_CLICKED)
        {
            char buffer[80];
            GetWindowText(hDatePicker, (LPTSTR)buffer, 80);
            SetWindowText(hLabelDate, buffer);
        }
        break;
    }
    return (INT_PTR)FALSE;
...
INT_PTR CALLBACK Login(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        if (LOWORD(wParam) == IDC_OK && HIWORD(wParam) == BN_CLICKED)
        {
            ...
            _login = true;
            strcpy(texto, "Bienvenido Dr. Jimenez al sistema de citas: ");
            strcat(texto, user);
